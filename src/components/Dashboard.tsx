...
import { ArrowUpCircle, ArrowDownCircle, DollarSign, PiggyBank, AlertTriangle } from 'lucide-react';
import { 
...
export const Dashboard = () => {
  const { transactions, config } = useFinance();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm flex flex-col justify-between">
          ...
          <div>
            <h3 className="text-2xl font-bold text-foreground">
              {formatCurrency(stats.mesAtualReceitas)}
            </h3>
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm flex flex-col justify-between">
          ...
          <div>
            <h3 className="text-2xl font-bold text-foreground">
              {formatCurrency(stats.mesAtualDespesas)}
            </h3>
          </div>
        </div>
      ...
      {!isMetaAtingida && stats.mesAtualDespesas > 0 && (
        <div className="bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 p-4 rounded-lg flex items-center gap-3">
          ...
          <p className="text-sm font-medium">Atenção: Sua economia deste mês está abaixo da meta estabelecida de {formatCurrency(config.metaEconomiaMensal)}.</p>
        </div>
      )}
      ...
      {categoryData.length > 0 && categoryData[0].value > (stats.mesAtualDespesas * 0.4) && (
        <div className="bg-danger/10 border border-danger/20 text-danger p-4 rounded-lg flex items-center gap-3">
          ...
          <p className="text-sm font-medium">
            Alerta: A categoria <strong>{categoryData[0].name}</strong> representa {Math.round((categoryData[0].value / stats.mesAtualDespesas) * 100)}% de seus gastos neste mês.
          </p>
        </div>
      )}

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Gráfico Bar - 12 meses */}
